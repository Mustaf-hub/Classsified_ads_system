...
$s = array();
	$p = array();
	$ps = array();
	$udata = $sellify->query("select * from tbl_user where id=".$profile_id."")->fetch_assoc();
	
	$ps['post_owner_id'] = $profile_id;
		$ps['owner_name'] = $udata['name'];
		$ps['owner_img'] = $udata['profile_pic'];
		$ps['owner_mobile'] = $udata['ccode'].$udata['mobile'];
...
$returnArr = array("ResponseCode"=>"200","Result"=>"true","ResponseMsg"=>"Home Data Get Successfully!!!","currency"=>$set['currency'],"searchlist"=>$p,"owner_Details"=>$ps);	
}
