...
if ($brand_id != 0) {
    $query .= " AND commercial_brand_id IN (".$brand_id.")";
}
...
// Conditionally append the brand_id clause if it's not zero
if ($brand_id != 0) {
    $query .= " AND sparepart_type_id IN (".$brand_id.")";
}
...
if ($brand_id != 0) {
    $query .= " AND bicycles_brand_id IN (".$brand_id.")";
}
...
$returnArr = array("ResponseCode"=>"200","Result"=>"true","ResponseMsg"=>"Commercial Filter Get Successfully!!!","filter"=>$p);	
}
