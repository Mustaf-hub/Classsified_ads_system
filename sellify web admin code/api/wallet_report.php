...
$checkimei = mysqli_num_rows(mysqli_query($sellify,"select * from tbl_user where  `id`=".$uid.""));

if($checkimei != 0)
    {
		$wallet = $sellify->query("select * from tbl_user where id=".$uid."")->fetch_assoc();
		
       $sel = $sellify->query("select message,status,amt from wallet_report where uid=".$uid." order by id desc");
$myarray = array();
$l=0;
$k=0;
$p = array();
