...
$query = "
    ...
    WHERE 
        ...
        AND is_expire = 0 
        AND uid != {$uid}
        AND post_type = 'job_post'
		AND subcat_id={$subcat_id}
";


	if ($job_position_type != 0) {
    $query .= " AND job_position_type IN('".$job_position_type."')";
}

if($job_salary_period !=0)
{
	$query .= " AND job_salary_period IN('".$job_salary_period."')";
}
...
$returnArr = array("ResponseCode"=>"200","Result"=>"true","ResponseMsg"=>"Job Category Filter Get Successfully!!!","filter"=>$p);	
}
