...
$km_end = $data['km_end'];
$owner_length = $data['owner_length'];
$fuel = $data['fuel'];
...
$query = "
    ...
    WHERE 
        ...
        AND is_expire = 0 
        AND uid != {$uid}
        AND post_type = 'sellify_post'
";

// Conditionally append the brand_id clause if it's not zero
if ($brand_id != 0) {
    $query .= " AND brand_id IN (".$brand_id.")";
}
...
if ($year_start != 0 && $year_end != 0) {
    $query .= " AND post_year BETWEEN {$year_start} AND {$year_end}";
}

if ($km_start != 0 && $km_end != 0) {
    $query .= " AND km_driven BETWEEN {$km_start} AND {$km_end}";
}

if($owner_length !=0)
{
	$query .= " AND no_owner IN('".$owner_length."')";
}

if($fuel !=0)
{
	$query .= " AND fuel IN('".$fuel."')";
}
...
$returnArr = array("ResponseCode"=>"200","Result"=>"true","ResponseMsg"=>"sellify Filter Get Successfully!!!","filter"=>$p);	
}
