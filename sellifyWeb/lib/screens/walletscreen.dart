...
class _WalletscreenState extends State<Walletscreen> {
  ...
  Future<void> getPriceZero() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();

    prefs.setString("amountOfWallet", "0");
  }
  @override
      ...
      builder: (context, constraints) {
        return Scaffold(
          ...
          appBar: PreferredSize(
              preferredSize: const Size.fromHeight(kIsWeb ? 75 : 0),
              child: kIsWeb ? CommonAppbar() : const SizedBox()),
          body: LayoutBuilder(
            builder: (context, constraints) {
                                ...
                                SizedBox(
                                  ...
                                  child: Stack(
                                    children: [
                                      SizedBox(
                                        width: double.infinity,
                                          child: SvgPicture.asset("assets/account_icons/walletbg.svg", fit: BoxFit.fill,height: 400,)),
                                      Padding(
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            IconButton(
                                              onPressed: () {
                                                Get.to(kIsWeb ? const HomePage() : const BottombarScreen(), transition: Transition.noTransition);
                                              },
                                              icon: Image.asset(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        walletreportCont.isloading ?
                                            ...
                                            : walletreportCont.walletData!.walletitem!.isNotEmpty ? ListView.separated(
              ...
              child: Container(
                ...
                child: Column(
                  children: [
                    ...
                    mainButton(containcolor: PurpleColor, context: context, txt1: "Continue".tr, onPressed1: () async {

                      SharedPreferences prefs = await SharedPreferences.getInstance();

                        prefs.setString("paymentfor", "WalletTopUp");
                        prefs.setString("amountOfWallet", amount.text.toString());
                      ...
                      if(amount.text.isNotEmpty){
                        if(selectedPayment == "Razorpay"){
                          ...
                          if(kIsWeb){
                           ...
                           paystackCont.getPaystack(amount: amount.text).then(
                            (value) {
                                                ...
                                                (value) {
                                                  if (value["Result"] ==
                                                    ...
                                                    walletreportCont
                                                      ...
                                                      (value) {
                                                        Get.to(
                                                            const Walletscreen(), transition: Transition.noTransition);
                                                      },
                                                    );
                                                  } else {
                                                    Get.back();
                         ...
                         if(kIsWeb) {
                          ...
                          Get.to(() => PaymentWebVIew(
                                    initialUrl:
                                        "${Config.paymentBaseUrl}paytm/index.php?amt=${amount.text}&uid=${getData.read("UserLogin")["id"]}",
                                    navigationDelegate:
                                        (NavigationRequest request) async {
                                      final uri = Uri.parse(request.url);
                        ...
                        if(kIsWeb){
                          webPaymentCont.getPaymentUrl(body: {
                            ...
                            Get.to(() => PaymentWebVIew(
                                    initialUrl:
                                        "${Config.paymentBaseUrl}result.php?detail=Movers&amount=${amount.text}&order_id=$notificationId&name=${getData.read("UserLogin")["name"]}&email=${getData.read("UserLogin")["email"]}&phone=${getData.read("UserLogin")["ccode"] + getData.read("UserLogin")["mobile"]}",
                                    navigationDelegate:
                                        (NavigationRequest request) async {
                                      final uri = Uri.parse(request.url);
                            ...
                            if (otid != null) {
                              topupWalletCont
                                  .getTopupWallet(amount: amount.text)
                                  .then(
                                (value) {
                                  if (value["Result"] == "true") {
                                    walletreportCont.getWalletReport().then(
                                      (value) {
                                        Get.to(const Walletscreen(), transition: Transition.noTransition);
                                      },
                                    );
                        ...
                        if(kIsWeb) {
                          ...
                          Get.to(() => PaymentWebVIew(
                                    initialUrl:
                                        "${Config.paymentBaseUrl}Payfast/index.php?amt=${amount.text}",
                                    navigationDelegate:
                                        (NavigationRequest request) async {
                                      final uri = Uri.parse(request.url);
                          ...
                          Get.to(() => PaymentWebVIew(
                                    initialUrl:
                                        "${Config.paymentBaseUrl}Midtrans/index.php?name=test&email=${getData.read("UserLogin")["email"]}&phone=${getData.read("UserLogin")["ccode"] + getData.read("UserLogin")["mobile"]}&amt=${amount.text}",
                                    navigationDelegate:
                                        (NavigationRequest request) async {
                                      final uri = Uri.parse(request.url);
                                      ...
                                      if (uri.queryParameters["transaction_status"] == null) {
                                        ...
                                        if (uri.queryParameters["transaction_status"] == "capture") {
                                          ...
                                          Get.back(
                                              result: uri.queryParameters["order_id"]);
                                        } else {
                                          Get.back();
                                          showToastMessage(
                                              "${uri.queryParameters["ResponseMsg"]}");
                         ...
                         if(kIsWeb) {
                          ...
                          Get.to(() => PaymentWebVIew(
                                    ...
                                    navigationDelegate:
                                      ...
                                      if (uri.queryParameters["status"] ==
                                          null) {
                                        accessToken = uri.queryParameters[
                                            "merchant_account_id"];
                                      } else {
                                        if (uri.queryParameters["status"] ==
                                            "Completed") {
