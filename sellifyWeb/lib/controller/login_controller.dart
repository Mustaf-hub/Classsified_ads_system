...
class LoginController extends GetxController implements GetxService {
  ...
  Future getLoginData(context) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (resultCheck == "true") {
        ...
        setDataFirebseLogin(name: getData.read("UserLogin")["name"],
            context: context,
            profilePic: getData.read("UserLogin")["profile_pic"],
            mobile: "${getData.read("UserLogin")["ccode"]}${getData.read(
                "UserLogin")["mobile"]}");
  ...
  void setDataFirebseLogin(
      {context, required String name, required String profilePic, required String mobile}) async {
    AuthService authService = AuthService();
    try {
      await authService.signInStroreData(name: name,
